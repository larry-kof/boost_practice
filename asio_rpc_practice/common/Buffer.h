...
//
// This is a public header file, it must only include public
// header files.
...
public:
    ...
    // are fine NOTE: implicit move-ctor is added in g++ 4.6
    ...
    // string str(retrieve(readableBytes()),
    // readableBytes()); the evaluation of two functions are
    // unspecified
    ...
    std::string toStringPiece() const {
        return std::string(
            peek(), static_cast< int >( readableBytes() ) );
    }
    ...
    void append( const void * /*restrict*/ data,
                 size_t len ) {
    ...
    void prepend( const void * /*restrict*/ data,
                  size_t len ) {
    ...
    void shrink( size_t reserve ) {
        // FIXME: use vector::shrink_to_fit() in C++ 11 if
        // possible.
        Buffer other;
...
private:
    ...
    void makeSpace( size_t len ) {
        if( writableBytes() + prependableBytes() <
            ...
            // move readable data to the front, make space
            // inside buffer
